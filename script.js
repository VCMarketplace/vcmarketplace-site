(() => {
  ...
  if (!prefersReducedMotion && hasGsap) {
    ...
    // Process line: animated on desktop; stable/static vertical timeline on mobile.
    const processArticles = [...document.querySelectorAll('.process-line article')];
    if (window.innerWidth > 760) {
      gsap.to('.process-line__progress', {
        ...
        scrollTrigger: {
          ...
          scrub: 1,
          onUpdate: self => {
            const activeCount = Math.max(1, Math.ceil(self.progress * processArticles.length));
            processArticles.forEach((article, index) => article.classList.toggle('is-active', index < activeCount));
          }
        }
      });
      ...
      processArticles.forEach(article => article.classList.add('is-active'));
      gsap.set('.process-line__progress', { clearProps: 'width', height: '100%' });
      gsap.set('.process-line article', { opacity: 1, y: 0, clearProps: 'transform' });
    }
